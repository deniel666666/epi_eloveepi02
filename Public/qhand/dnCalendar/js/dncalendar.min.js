...
if(typeof settings.notes!=='undefined' && settings.notes!==''){if(dateIsNotes(new Date(year,month-1,date))){colDateClass+=" note "}}
...
// dateNumArr.push(date);// 
	...
	if(notes[i].notes){
		var noteListLength=noteList.length;var list="";list+="<li class='date'>";list+="<span>"+date+"</span> ";if(noteListLength>0){list+=" : ";for(var j=0;j<noteListLength;j++){list+=noteList[j];if(noteListLength<=j){list+=", "} } }
