    ...
    if(C('URL_MODEL') == 0) { // 普通模式URL转换
        ...
        if(!empty($vars)) { // 添加参数
            if($suffix){
                ...
                $vars_params = [];
                foreach ($vars as $var => $val){
                    if('' !== trim($val))   array_push($vars_params, $var . '=' . urlencode($val));
                }
                if(count($vars_params)>0){
                    $url .= '?' . implode('&', $vars_params);
                }
            }
        }
        if($suffix) {
...
function send_email_user_id($Body='test', $user_id=0, $subject=NULL){
    $user = D('eip_user')->where('id="'.$user_id.'"')->find();
    $address = $user ? $user['email'] : '';
    return send_email($Body, $address, $subject);
}
function send_email($Body='test', $address='andyplanner@photonic.com.tw', $subject=NULL){
    if(!$address){ return; }
    ...
    if($mail){
        ...
        $mail->Password = MAIL_FROM_PASSWROD;
        $mail->Subject = MAIL_FROM_TITLE." ".$subject;
        $mail->From = MAIL_FROM_ADDRESS;
        $mail->SMTPAutoTLS = false;
        $mail->FromName = MAIL_FROM_ADDRESS;
        $mail->Body = $Body;
        $mail->IsHTML(true);
        ...
        $subject=mb_encode_mimeheader(MAIL_FROM_TITLE." ".$subject, 'UTF-8');
        mb_internal_encoding('UTF-8');
